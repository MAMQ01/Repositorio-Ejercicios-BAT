...
echo 5) Arrancar LibreOffice Calc
echo.
...
if !opcion! EQU 1 (
	...
	echo QUINTA OPCION = HOJA DE CALCULO
	start /D "C:\Program Files\LibreOffice\program\" scalc.exe
	goto :iniciomenu3
) else (
...
echo ----------------------------------
echo Manuel MUNOZ ENTREGA - 8
echo 15.1.2024 Lunes
echo ----------------------------------

rem /D y separado el directorio para abrir un ejecutable que no está en la variable PATH
rem type y ejecutar el documento logotipoPorEjemplo.ans
rem manytools para convertir de img a ansi art seleccionar use color solo esa.
rem dir .*ans
